...
export default function SourceSchemasPage() {
  ...
  const [previewSchemaId, setPreviewSchemaId] = useState<string | null>(
    schemaIdFromUrl,
  );
  ...
  const handleBulkDeleteConfirm = useCallback(async () => {
    ...
    try {
      ...
      toast.success(
        "Source schemas deleted",
        `${bulkDeleteIds.length} source schema${bulkDeleteIds.length > 1 ? "s" : ""} have been removed.`,
      );
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                <div className="font-medium">
                  {schema.name || schema.sourceTable || "Unnamed"}
                </div>
            ...
            <div className="flex items-center justify-end gap-2">
              ...
              <DropdownMenu>
                ...
                <DropdownMenuContent align="end">
                  <DropdownMenuItem
                    onClick={() => setPreviewSchemaId(schema.id)}
                  >
  ...
  return (
    <div className="space-y-6">
      ...
      <ConfirmationModal
        ...
        action="delete"
        itemName="Source Schema"
        title={`Delete ${bulkDeleteIds?.length ?? 0} Source Schema${(bulkDeleteIds?.length ?? 0) > 1 ? "s" : ""}`}
        description={`Are you sure you want to delete ${bulkDeleteIds?.length ?? 0} selected source schema${(bulkDeleteIds?.length ?? 0) > 1 ? "s" : ""}? This action cannot be undone.`}
...
function SourceSchemaPreviewDialog({
  ...
  const { data: previewData, isLoading } = usePreviewSourceData(
    organizationId,
    schemaId || undefined,
    10,
  );
  ...
  const handleDiscover = async () => {
    try {
      ...
      toast.success(
        "Schema discovered",
        "Column information has been updated.",
      );
    } catch (error) {
      toast.error(
        "Discovery failed",
