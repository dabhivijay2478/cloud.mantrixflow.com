...
import { PawPrint, Trees, Satellite, Menu, X, Youtube, Instagram } from "lucide-react"
    ...
    const observer = new IntersectionObserver(
      ([entry]) => {
        if (entry.isIntersecting) {
          // Extract numeric value from string (e.g., "2.4M" -> 2.4, "12K+" -> 12)
          const numericStr = value.replace(/[^0-9.]/g, "")
          const targetNum = Number.parseFloat(numericStr)
          const unit = value.replace(/[0-9.]/g, "") // "M", "K+", "%", etc.

          let current = 0
          const increment = targetNum / 60 // Animate over ~1 second (60 frames)
...
export default function TerraPage() {
  ...
  useEffect(() => {
    const handleScroll = () => {
      ...
      if (dashboardRef.current) {
        ...
        // Dashboard starts rotating when it enters the viewport
        // and completes at 45° when it reaches 20% from top
        const rotationStart = viewportHeight * 0.8 // Start rotation when dashboard is 80% down viewport
        const rotationEnd = viewportHeight * 0.2 // Complete rotation at 20% from top

        if (dashboardRect.top >= rotationStart) {
          // Dashboard hasn't started rotating yet - stays at 0°
          setDashboardScrollOffset(0)
        } else if (dashboardRect.top <= rotationEnd) {
          // Rotation complete - stays at 45°
          setDashboardScrollOffset(15)
        } else {
          // Progressive rotation between 0° and 45°
  ...
  useEffect(() => {
    const featuresCount = 4 // Number of features
  ...
  return (
    <div className="relative min-h-screen bg-[#0B0C0F] text-[#F2F3F5] overflow-x-hidden">
      <header className="fixed top-6 left-6 md:w-auto md:right-auto right-6 z-40 border border-white/10 backdrop-blur-md bg-[#0B0C0F]/80 rounded-[16px]">
        <div className="w-full mx-auto px-6">
          <div className="flex items-center gap-6 md:h-14 h-14">
            ...
            {/* Desktop Navigation Menu - Shows on desktop with 24px gap from logo */}
      ...
      {/* Section 3 — Product Narrative (Editorial) */}
        ...
        <div className="max-w-[1120px] w-full mx-auto">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-12 md:gap-16 items-stretch">
            ...
            <div className="max-w-[720px]">
              ...
              {/* Features List with Clickable Buttons */}
            ...
            {/* Desktop Image Stack with Perspective Effect */}
            <div className="hidden md:flex items-stretch justify-center">
              <div className="relative w-full h-full min-h-[500px]">
                {[
                  ...
                  // Calculate position in stack based on selected feature
      ...
      {/* Section 4 — Alternate Theme Hero (Gradient + CTA) */}
      <section
        id="cta"
